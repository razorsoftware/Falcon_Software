...
namespace Zenfox_Software.caixa
{
    public partial class Fechamento_Caixa : Form
    {
        ...
        public Double total_vendas_liquido = 0;
        public Double valor_fechamento = 0;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Deseja realmente fechar este caixa ? esta ação não poderá ser desfeita !", "Confirmar", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                if (MessageBox.Show("Deseja imprimir o cupom de fechamento de caixa ?", "Confirmar", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes){
                    ...
                    impressora.crediario_vendido = this.crediario_vendido;
                    impressora.crediario_recebido = this.crediario_recebido;
                    ...
                    impressora.total_vendas = this.total_vendas;
              ...
              //      impressora.total_geral = (this.valor_dinheiro + this.valor_credito + this.valor_debito + this.valor_cheque );
                    ...
                    impressora.total_desconto = this.valor_desconto;
        ...
        public void lista_totais()
        {
            ...
            this.total_vendas = (this.valor_dinheiro + this.valor_debito + this.valor_credito + this.valor_cheque);
            lbl_total_vendas.Text = this.total_vendas.ToString("F2");


            this.total_vendas_liquido = (this.total_vendas) - (this.valor_desconto + this.valor_cancelado);
            lbl_total_liquido.Text = this.total_vendas_liquido.ToString("F2");


            this.valor_fechamento = (this.total_vendas_liquido + this.valor_abertura + this.valor_suprimento_caixa) - (this.valor_retirada_de_caixa ) ;
            lbl_total_geral.Text = "R$ " + this.valor_fechamento.ToString("F2");
            //
        ...
        private void lbl_total_desconto_Click(object sender, EventArgs e)
        {
